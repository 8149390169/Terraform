...
resource "aws_launch_configuration" "launch-configuration" {
  name = "${var.launch-configuration-name}-${var.environment}"
...
resource "aws_autoscaling_group" "autoscaling-group" {
  name = "${var.autoscaling-group-name}-${var.environment}"
  ...
  vpc_zone_identifier = flatten(["${var.vpc-zone-identifier}"])
  health_check_type = "${var.health-check-type}"
  target_group_arns = "${var.target-group-arns}"
#  load_balancers = ["${var.load-balancers-arn}"]

  tag {
    key = "${var.tag-key}"
    propagate_at_launch = true
    value = "${var.launch-configuration-name}-${var.environment}"
  }
}
...
resource "aws_autoscaling_policy" "auto-scaling-policy-scale-up" {
  ...
  name = "${var.auto-scaling-policy-name-scale-up}-${var.environment}"
...
resource "aws_cloudwatch_metric_alarm" "cpu-alarm-scale-up" {
  alarm_name = "${var.alarm-name-scale-up}-${var.environment}"
...
resource "aws_autoscaling_policy" "auto-scaling-policy-scale-down" {
  autoscaling_group_name = "${aws_autoscaling_group.autoscaling-group.name}"
  name = "${var.auto-scaling-policy-name-scale-down}-${var.environment}"
...
resource "aws_cloudwatch_metric_alarm" "cpu-alarm-scale-down" {
  alarm_name = "${var.alarm-name-scale-down}-${var.environment}"
